...
The signed-in actor boundary is incomplete for Connect and withdrawal. Reviewed comments bind a configured installation actor to a browser `Me` identity check, but profile observation and invitation inspection do not yet report that check, and their action authorization does not require it. A wrong signed-in account could therefore pass target-recipient checks if it held a valid installation token. This requires an extension and server change with authenticated browser verification before any live Connect or withdrawal acceptance. No such live action has been attempted.
