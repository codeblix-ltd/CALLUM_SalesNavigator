...
During development, `shadow` uses read-only V1 lead rows and V2-only run/command rows. V2 assignments do not reserve leads against V1. `live_canary` requires an exact configured QA profile key, one lead ID, and an enabled V2 installation owned by the selected operator. It is disabled when the QA configuration is absent. It rejects a lead already in `public.lead_assignments` at run creation and again before an irreversible action is authorized. V1 can still assign a lead after the final read; [the ownership cutover design](lead-ownership-cutover.md) records the coordinated protocol required before broader live rollout.

The final command ACK completes a run when every lead is complete and no command or action intent remains unresolved. An explicit admin request for another read-only inspection, or a reviewed QA comment/withdrawal following a fresh inspection, may reopen a completed run in the same transaction that queues the command. Each completion cycle and reopening has its own event. A paused or failed run cannot be reopened this way. Read-only inspection requests can be repeated without reusing a completed command.
