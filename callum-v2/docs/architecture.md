...
During development, `shadow` uses read-only V1 lead rows and V2-only run/command rows. V2 assignments do not reserve leads against V1. `live_canary` requires an exact configured QA profile key and one lead ID, and is disabled when that configuration is absent.
