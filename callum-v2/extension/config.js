globalThis.CallumConfig = Object.freeze({
  validate(x) {
    ...
    const optional = ['postScope','postLink','commentButton','contactLink','contactDialog','contactEmail'];
    ...
    if (!x.labels || typeof x.labels !== 'object' || Object.keys(x.labels).some(k => !['connect','more','pending','connected','send','contactInfo'].includes(k))) throw new Error('CONFIG_INVALID');
