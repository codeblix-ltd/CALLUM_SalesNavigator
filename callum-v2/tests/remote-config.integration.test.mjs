...
test('server hotfix, rollback and operator kill apply to pending commands', {skip:!enabled}, async()=>{
  ...
  let version=null,previousConfig=null,installationId=null;
  const runIds=[];
  try {
    ...
    const issued=await s.createInstallation(op,CURRENT_EXTENSION_VERSION,'68f5971a'),installation=await s.installation(issued.token);
    installationId=issued.id;
    const before=await s.createRun({operatorId:op,mode:'shadow',count:1});
    runIds.push(before.id);
    ...
    for(const id of runIds)await s.pauseRun(id).catch(()=>{});
    if(installationId)await s.revokeInstallation(installationId).catch(()=>{});
    await s.disableOperator(op,true).catch(()=>{});
    await db.close();
  }
});
