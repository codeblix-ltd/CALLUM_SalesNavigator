...
test('Cockroach V2 command lease, lost ACK, reconciliation, and duplicate pay invariant', { skip: !enabled }, async () => {
  ...
  try {
    await s.seed();await s.createOperator(operatorId,'dev',3);
    const issued=await s.createInstallation(operatorId,'2.0.0','68f5971a');
    ...
    // Zero-amount fixture checks derivation and uniqueness without defining a compensation rate.
