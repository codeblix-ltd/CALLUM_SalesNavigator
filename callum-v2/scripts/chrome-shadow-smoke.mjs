...
const extensionPath=resolve(process.argv[2] || 'extension');
...
try {
  ...
  const issued=await control.createInstallation(operatorId,'2.0.0','68f5971a');
  ...
  console.log(JSON.stringify({extensionId:id,runId:run.id,commandType:command?.type,commandStatus:command?.status,
    ...
    assignmentStage:assignment?.stage,events:events.map(x=>x.event_type),popupResponse:response.slice(0,800),hotfix,
