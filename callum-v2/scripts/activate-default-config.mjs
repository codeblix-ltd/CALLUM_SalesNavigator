...
try {
  ...
  await control.seed();
  const { rows } = await db.query('SELECT version,config FROM callum_v2.remote_configs ORDER BY version DESC');
  const existing = rows.find(row => isDeepStrictEqual(row.config, DEFAULT_CONFIG));
  const version = existing ? Number(existing.version) : Number((await control.createConfig(DEFAULT_CONFIG)).version);
