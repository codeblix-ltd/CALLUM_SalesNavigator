...
try {
  ...
  const existing = rows.find(row => isDeepStrictEqual(row.config, DEFAULT_CONFIG) && row.min_extension_version === '2.3.0');
  const version = existing ? Number(existing.version) : Number((await control.createConfig(DEFAULT_CONFIG,'2.3.0')).version);
