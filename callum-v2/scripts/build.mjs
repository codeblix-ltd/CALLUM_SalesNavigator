...
await writeFile(join(destination, 'extension', 'build-info.js'),
  `globalThis.CALLUM_V2_BUILD = Object.freeze({ sha: '${sha}', protocol: 1, adapter: 1, environment: 'canary' });\n`);
...
console.log(JSON.stringify({ web: join(destination,'web'), extension: join(destination,'extension'), buildSha: sha, version: '2.0.0' }));
