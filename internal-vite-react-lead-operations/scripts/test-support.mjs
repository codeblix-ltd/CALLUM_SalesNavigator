...
assert.equal(pauseSandbox.pauseReportContext({status:'stopped',pauseDetails}).pauseStage,'Sent invitations','Stop must not erase the last pause evidence');
...
const issueDetails={kind:'page_unavailable',stage:'Ameera Ashraf recent activity',pageUrl:'https://www.linkedin.com/in/ameera-ashraf-9880/recent-activity/all/?token=SECRET',expectedUrl:'https://example.com/private',occurredAt:Date.now()-1000};
const issueContext=pauseSandbox.leadIssueReportContext({status:'stopped',lastLeadIssue:issueDetails});
assert.equal(issueContext.leadIssueStage,'Ameera Ashraf recent activity');
assert.equal(issueContext.leadIssuePageUrl,'https://www.linkedin.com/in/ameera-ashraf-9880/recent-activity/all/');
assert.equal(issueContext.leadIssueExpectedUrl,'');
let sanitized;
...
await action({...ctx,runMutation:async(_ref,args)=>{sanitized=args.context;return {id:'report',inserted:true};}}, {...valid,clientId:'d7b8097f-43b9-47d8-9873-5d22beab45ab',screenshots:[],context:{...valid.context,...issueContext,leadIssuePageUrl:issueDetails.pageUrl}});
assert.equal(sanitized.leadIssuePageUrl,'https://www.linkedin.com/in/ameera-ashraf-9880/recent-activity/all/');
