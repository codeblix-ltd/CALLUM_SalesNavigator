...
const shim = `globalThis.chrome={runtime:{getManifest:()=>({version:'0.10.36'})},storage:{local:{get:async()=>({autoLeadRunState:{status:'paused',message:'Checking whether you are already connected',currentLead:{fullName:'Example lead'}}})}},tabs:{get:async()=>({url:'https://www.linkedin.com/in/example'})}};
const reports=[{_id:'preview-report',description:'The run paused on a profile.',status:'investigating',messages:[{author:'support',text:'We found a profile check problem. Please try the update when it is available.',sentAt:Date.now()}],screenshotCount:0}];
globalThis.ScoutApi={getAuth:async()=>({token:'preview-only'}),authenticatedQuery:async()=>({page:reports,isDone:true,continueCursor:''}),authenticatedMutation:async(_path,args)=>{reports[0].messages.push({author:'scout',text:args.text,sentAt:Date.now()});return null;},authenticatedAction:async()=>{await new Promise(r=>setTimeout(r,400));return 'LOCAL-PREVIEW-NOT-SENT';}};`;
...
document.getElementById('run').onclick=async()=>{
 ...
 try {
  ...
  if (!["report.html", "report.js", "report.css", "reports.html", "reports.js"].includes(name)) { res.writeHead(404); return res.end(); }
