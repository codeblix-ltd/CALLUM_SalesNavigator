...
assert.equal(manifest.version, "0.10.41");
...
assert.match(popupScript, /const heldFailed = Math\.max/);
assert.match(backgroundSource, /dashboard\.counts\.retryableFailed \?\? dashboard\.counts\.failed/);
assert.match(scoutSource, /AS retryable_failed/);
assert.match(scoutSource, /AS needs_review/);
...
assert.match(adminSource, /export const getGhlSyncHistory/);
assert.match(adminSource, /ghl_contact_id/);
...
assert.match(ghlDeliverySource, /export const auditContactLinks = internalAction/);
...
assert.match(ghlSource, /delivery_outcome = \$3/);
assert.match(ghlSource, /export async function auditGhlOutboxRows/);
...
assert.match(schemaSource, /ADD COLUMN IF NOT EXISTS ghl_contact_id/);
assert.match(adminAppSource, /function GhlSyncHistoryPanel/);
assert.match(adminAppSource, /Every contact delivery, in one place/);
