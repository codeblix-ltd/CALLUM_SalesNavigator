...
function harness(rows) {
 const sql=[],writes=[];
 const held = row => row.last_error===rules.UNVERIFIED_PROFILE_LINK_ERROR && /\/in\/AC[ow][A-Za-z0-9_-]{15,}\/?([?#].*)?$/.test(row.linkedin_url);
 const query = async (text,args=[]) => {
  ...
  if (text.includes('AS profile_link_needs_review'))return {rows:rows.filter(r=>r.id===args[1]).map(r=>({...r,profile_link_needs_review:held(r)}))};
  assert(text.includes(rules.profileLinkNeedsReviewSql()),'every automatic selection uses the same guard');
...
for (const args of [{},{failedOnly:true}]) {
 ...
 await assert.rejects(heldOnly.run(args),/remaining leads need profile-link review/);
...
assert.match(source.slice(source.indexOf('export const getDashboard'),source.indexOf('export const claimNextLead')),/AND NOT \(\$\{profileLinkNeedsReviewSql\(\)\}\)/);
assert.match(source,/stage === "needs_attention"[\s\S]{0,230}OR \(\$\{profileLinkNeedsReviewSql\(\)\}\)/);
assert.match(source,/stage === "automation_ready"[\s\S]{0,350}AND NOT \(\$\{profileLinkNeedsReviewSql\(\)\}\)/);
...
if(process.argv.includes('--live')) {
 ...
 try {
  ...
  console.log('PASS: 9 real read-only Cockroach predicate cases');
 }finally{await c.end();}
}
